...
this.json;this.data=[];this.bar={height:700,margin:{top:20,right:20,bottom:200,left:40},valueline:null};this.gauge={value:null,max:null};this.line={height:700,margin:{top:50,right:20,bottom:100,left:60},valueline:null,periods:[10,30],maxVal:{date:null,total:0},minVal:{date:null,total:99999999999}};
...
taskListObj=new TaskListObj();carouselObj=new CarouselObj();calendarObj=new CalendarObj();$(document).ready(function(){modalObj.init();mainMenuObj.init();
listViewObj.init();formObj.init();taskListObj.init();calendarObj.init();});ModalObj.prototype.init=function(){$(".open-modal").click(function(a){a.preventDefault();
...
}}c["render"+c.graphType.charAt(0).toUpperCase()+c.graphType.slice(1)+"Graph"]();};ReportObj.prototype.getGraphData=function(){var b=this;b.data=[];if(b.graphType=="bar"){b.json.forEach(function(c){b.data.push({variable:+c[b.axis.yAxisIndex],xAxisIndex:c[b.axis.xAxisIndex]});
});}console.log(b.data);if(b.graphType=="line"){var a=new Date();a.setDate(a.getDate()-b.line.periods[0]-2);b.json.forEach(function(e){var c=new Date(e.date);
if(c.getTime()>=a.getTime()){b.data.push({date:e[b.axis.xAxisIndex],total:+e[b.axis.yAxisIndex]});if(+e[b.axis.yAxisIndex]>b.line.maxVal.total){b.line.maxVal={date:e[b.axis.xAxisIndex],total:+e[b.axis.yAxisIndex]};
}if(+e[b.axis.yAxisIndex]<b.line.minVal.total){b.line.minVal={date:e[b.axis.xAxisIndex],total:+e[b.axis.yAxisIndex]};}}});}};ReportObj.prototype.renderBarGraph=function(){var a=this;
...
};ReportObj.prototype.updateLineGraph=function(e){var h=this,b=[],c=new Date(),g={date:null,total:0},f={date:null,total:99999999999};c.setDate(c.getDate()-h.line.periods[e]-2);
h.json.forEach(function(m){var l=new Date(m.date);if(l.getTime()>=c.getTime()){b.push({date:m.date,total:+m.total});if(+m.total>g.total){g={date:m.date,total:+m.total};
}if(+m.total<f.total){f={date:m.date,total:+m.total};}}});h.axis.x.domain([new Date(b[0].date),new Date(b[b.length-1].date)]);h.axis.y.domain([0,d3.max(b,function(l){return l.total;
})]);h.axis.xAxis.scale(h.axis.x).ticks(d3.time.days,Math.ceil(h.data.length/4));var k=d3.select("#graph").transition(),d=h.svg.transition().duration(750),a=function(m,l){return l*50;
};h.svg.selectAll(".x.axis").call(h.axis.xAxis).selectAll("text").style("text-anchor","end").attr("x",-10).attr("dy","1em").attr("transform",function(l){return"rotate(-90)";
});d.select(".y.axis").call(h.axis.yAxis);h.svg.selectAll(".graph-line").data(b).transition().duration(750).attr("d",h.line.valueline(b));h.svg.selectAll(".max-text").data([g]).text(function(n,l){var m=d3.time.format("%b %d");
return n.total+" on "+m(new Date(n.date));});h.svg.selectAll(".min-text").data([f]).text(function(n,l){var m=d3.time.format("%b %d");return n.total+" on "+m(new Date(n.date));
